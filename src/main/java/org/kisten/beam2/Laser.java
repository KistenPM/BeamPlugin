        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (!checkRadius) {
                    ...
                    new ParticleBuilder(Particle.REDSTONE).allPlayers().location(laserLocation).offset(.25, .25, .25).allPlayers().extra(0.1).color(Color.RED).count(6).spawn();
                    if (1 >= laserLocation.distance(Destination)) { // при точке назначения
                        new ParticleBuilder(Particle.REDSTONE).allPlayers().location(Destination).offset(.5, .5, .5).allPlayers().extra(0.1).color(Color.ORANGE).count(20).spawn();
                        new ParticleBuilder(Particle.REDSTONE).allPlayers().location(Destination).offset(.5, .5, .5).allPlayers().extra(0.1).color(Color.YELLOW).count(40).spawn();
                        cancel();
                    }
                    laserLocation.add(travel1);
                    if (distance >= numTrips1) {
                        cancel();
                    }
                }
                else {
                    if (startLocation.distance(perp) > startLocation.distance(Destination) && direction.angle(toStart.normalize()) == 0) { // после точки назначения
                        if (distance == 1) {
                            finalTravel.multiply(-1);
                        }
                        new ParticleBuilder(Particle.REDSTONE).allPlayers().location(Edge2).offset(.25, .25, .25).allPlayers().extra(0.1).color(Color.RED).count(4).spawn();
                        Edge2.add(finalTravel);
                        if (1 >= Edge2.distance(Destination)) {
                            new ParticleBuilder(Particle.REDSTONE).allPlayers().location(Edge2).offset(.5, .5, .5).allPlayers().extra(0.1).color(Color.ORANGE).count(20).spawn();
                            new ParticleBuilder(Particle.REDSTONE).allPlayers().location(Edge2).offset(.5, .5, .5).allPlayers().extra(0.1).color(Color.YELLOW).count(40).spawn();
                            cancel();
                        }
                    } else { // между точкой начала и концом
                        new ParticleBuilder(Particle.REDSTONE).allPlayers().location(Edge).offset(.25, .25, .25).allPlayers().extra(0.1).color(Color.RED).count(4).spawn();
                        Edge.add(finalTravel);
                        if (1 >= Edge.distance(Destination)) { // при точке назначения
                            new ParticleBuilder(Particle.REDSTONE).allPlayers().location(Edge).offset(.5, .5, .5).allPlayers().extra(0.1).color(Color.ORANGE).count(20).spawn();
                            new ParticleBuilder(Particle.REDSTONE).allPlayers().location(Edge).offset(.5, .5, .5).allPlayers().extra(0.1).color(Color.YELLOW).count(40).spawn();
                            cancel();
                        }
                    }
                }
